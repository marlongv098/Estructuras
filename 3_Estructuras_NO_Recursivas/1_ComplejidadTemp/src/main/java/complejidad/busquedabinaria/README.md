...
## Código en Java  
```java
package complejidad.busquedabinaria;
...
* La versión **iterativa** (la implementada aquí) solo usa tres variables (`inicio`, `fin`, `medio`) sin importar el tamaño de la entrada: **espacio O(1)**.
* Si se implementara de forma **recursiva** (una llamada por cada mitad descartada), cada llamada agrega un frame a la pila, dando **espacio O(log n)** en vez de O(1) — un ejemplo claro de que la misma complejidad temporal (O(log n)) puede tener distinto costo espacial según cómo se implemente.
